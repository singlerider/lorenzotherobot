...
def check(args):
    ...
    if args[0] == 'trades':
        ...
        all_tradable_pokemon_comprehension = ["{}".format(
            user) for user, giverpoke, pos, askingpoke, askinglevel in all_tradable_pokemon]
        ...
        if len(user_tradable_pokemon) > 0:
            user_tradable_pokemon_comprehension = ["({}) {}, {}, lvl {})".format(int(pos), str(giverpoke), str(
                askingpoke), int(askinglevel)) for user, giverpoke, pos, askingpoke, askinglevel in user_tradable_pokemon]
