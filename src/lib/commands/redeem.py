...
def redeem(args):        
        ...
        if len(tradable_pokemon) >= 0:
            ...
            receiver_trade_status = get_receiver_trade_status(position_to_trade, globals.CURRENT_USER)
            giver_trade_status = get_giver_trade_status(position_to_redeem, username_to_trade)
            ...
            if required_id == receiver_id:
                if receiver_level >= required_level:
                    trade_transaction(username_to_trade, position_to_redeem, globals.CURRENT_USER, position_to_trade)
