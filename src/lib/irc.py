...
class GroupChat(Thread):  # pragma: no cover
    ...
    def nextMessage(self):
        ...
        print ">>", line
        if line.startswith("PING"):
            self.whisper.send(line.replace("PING", "PONG") + "\r\n")
    ...
    def check_for_ping(self, data):
        ...
        # if data[0:4] == "PING":
        if data.find('PING') != -1:
            self.whisper.send('PONG ' + data.split()[1] + '\r\n')
            last_ping = time.time()
        if (time.time() - last_ping) > threshold:
            sys.exit()
    ...
    def group_chat(self):
        ...
        try:
            sock.connect(('199.9.253.120', 6667))
        ...
        sock.send('PRIVMSG #jtv :/w ' + 'singlerider' + ' ' + 'HeyGuys' + "\r\n")
        self.whisper = sock
    ...
    def join_channels(self, channels):
        pp('Joining channels %s.' % channels)
        self.whisper.send('JOIN %s\r\n' % channels)
        pp('Joined channels.')

    def leave_channels(self, channels):
        pp('Leaving channels %s,' % channels)
        self.whisper.send('PART %s\r\n' % channels)
        pp('Left channels.')
