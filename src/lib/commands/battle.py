...
def battle(args):
    ...
    now = datetime.utcnow()
    required_cooldown =  datetime.utcnow() - timedelta(minutes=10)
    ...
    if get_last_battle(globals.CURRENT_USER) == None:
        ...
        if now > last_battle_time + timedelta(minutes=10):
            ...
            return "Don't even try it. You have to wait ten minutes between battles"
